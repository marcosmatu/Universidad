...
void GSimulation :: get_acceleration(sycl::queue Q,int n)
{
 /* 
   int i,j;
...
real_type GSimulation :: updateParticles(sycl::queue Q,int n, real_type dt)
{
  /*
   int i;
   real_type energy = 0;
...
void GSimulation :: start() 
{
  ...
  particles->pos_x = sycl::malloc_shared<real_type>(n, Q);
  particles->pos_y = sycl::malloc_shared<real_type>(n, Q);
  particles->pos_z = sycl::malloc_shared<real_type>(n, Q);
  particles->vel_x = sycl::malloc_shared<real_type>(n, Q);
  particles->vel_y = sycl::malloc_shared<real_type>(n, Q);
  particles->vel_z = sycl::malloc_shared<real_type>(n, Q);
  particles->acc_x = sycl::malloc_shared<real_type>(n, Q);
  particles->acc_y = sycl::malloc_shared<real_type>(n, Q);
  particles->acc_z = sycl::malloc_shared<real_type>(n, Q);
  particles->mass  = sycl::malloc_shared<real_type>(n, Q);
  ...
  {   
   ts0 += time.start(); 
    ...
    get_acceleration(Q,n);

    energy = updateParticles(Q,n, dt);
    _kenergy = 0.5 * energy; 
